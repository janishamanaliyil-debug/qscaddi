...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Chat URL - regular chat endpoint
    const CHAT_URL = window.location.pathname; // Uses current page URL
    ...
    function handleNormalSubmission(message) {
        console.log('=== Normal Chat Submission ===');
        console.log('Message:', message);
        console.log('Files:', selectedFiles.length);
        
        if (message === '' && selectedFiles.length === 0) {
            console.log('Empty submission, returning');
            return;
        }
        
        // Hide greeting and ensure chat area is ready
        if (centerGreeting) {
            centerGreeting.classList.add('hidden');
            console.log('Greeting hidden');
        }
        
        // Ensure chat area is in proper mode
        if (chatArea) {
            chatArea.style.justifyContent = 'flex-start';
            chatArea.style.alignItems = 'flex-start';
            console.log('Chat area styling updated');
        }
        ...
        // Send to current page (index handles POST)
        ...
        .then(data => {
            ...
            if (data.success && data.response) {
                console.log('Displaying bot response (success path):', data.response);
                displayBotResponse(data.response);
            } else if (data.response) {
                console.log('Displaying fallback response:', data.response);
                displayBotResponse(data.response);
            } else {
                console.error('No response in data:', data);
                displayBotResponse('Sorry, I could not process your request.');
            }
            
            // Handle uploaded files info if present
            if (data.uploaded_files && data.uploaded_files.length > 0) {
                console.log('Files uploaded:', data.uploaded_files);
            }
        })
        .catch(err => {
            ...
            hideLoadingIndicator();
            displayBotResponse('Sorry, there was an error processing your message. Please try again.');
        });
    ...
    function displayBotResponse(text) {
        ...
        console.log('displayBotResponse called with:', text);
        
        const messageDiv = document.createElement('div');
        messageDiv.className = 'chat-message bot-message';
        messageDiv.innerHTML = `<div class="message-text">${formatBotMessage(text)}</div>`;
        
        console.log('Appending bot message to chat area');
        chatArea.appendChild(messageDiv);
        chatArea.scrollTop = chatArea.scrollHeight;
        
        console.log('Bot message displayed successfully');
    }
    ...
    function displayDownloadLink(fileUrl) {
        ...
        const linkDiv = document.createElement('div');
        linkDiv.className = 'chat-message bot-message';
        linkDiv.innerHTML = `
            <div class="message-text">
                <a href="${fileUrl}" download class="download-link">
                    <i class="bi bi-download"></i> Download File
    ...
    function showLoadingIndicator() {
        ...
        const loadingDiv = document.createElement('div');
        loadingDiv.className = 'chat-message bot-message loading-indicator';
        loadingDiv.id = 'loadingIndicator';
        loadingDiv.innerHTML = `
            <div class="message-text">
    ...
    console.log('chatArea exists:', !!chatArea);
